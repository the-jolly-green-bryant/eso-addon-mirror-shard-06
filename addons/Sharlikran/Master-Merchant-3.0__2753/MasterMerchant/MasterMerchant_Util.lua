...
MasterMerchant.shellGaps = { 3938, 1750, 701, 301, 132, 57, 23, 10, 4, 1, }
...
function MasterMerchant.shellSort(inputTable, comparison, numElements)
  ...
  if numElements >= MM_SCROLL_LIST_USE_TABLE_SORT_MIN then
    table.sort(inputTable, comparison)
    return inputTable
  end
  for _, gapVal in ipairs(MasterMerchant.shellGaps) do
    for i = gapVal + 1, numElements do
      ...
      for j = i - gapVal, 1, -gapVal do
        ...
        inputTable[insertPos] = testVal
        insertPos = j
      end
      inputTable[insertPos] = tableVal
    end
  end
  return inputTable
end
