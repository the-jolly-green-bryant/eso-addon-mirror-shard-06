...
local function CheckFilterTimeframe(timestamp)
  ...
  if MasterMerchant.systemSavedVariables.windowTimeRange == LGS_WINDOW_TIME_RANGE_DEFAULT then
    timeframeStart = MasterMerchant:CheckTimeframe()
    if (timestamp > timeframeStart) then validRange = true end
  end
  if MasterMerchant.systemSavedVariables.windowTimeRange == LGS_WINDOW_TIME_RANGE_THIRTY then
    timeframeStart = LGSTime.filterDateRanges[LGS_WINDOW_TIME_RANGE_THIRTY].startTimestamp
    timeframeEnd = LGSTime.filterDateRanges[LGS_WINDOW_TIME_RANGE_THIRTY].endTimestamp
    if (timestamp >= timeframeStart and timestamp < timeframeEnd) then validRange = true end
  end
  if MasterMerchant.systemSavedVariables.windowTimeRange == LGS_WINDOW_TIME_RANGE_SIXTY then
    timeframeStart = LGSTime.filterDateRanges[LGS_WINDOW_TIME_RANGE_SIXTY].startTimestamp
    timeframeEnd = LGSTime.filterDateRanges[LGS_WINDOW_TIME_RANGE_SIXTY].endTimestamp
    if (timestamp >= timeframeStart and timestamp < timeframeEnd) then validRange = true end
  end
  if MasterMerchant.systemSavedVariables.windowTimeRange == LGS_WINDOW_TIME_RANGE_NINETY then
    timeframeStart = LGSTime.filterDateRanges[LGS_WINDOW_TIME_RANGE_NINETY].startTimestamp
    timeframeEnd = LGSTime.filterDateRanges[LGS_WINDOW_TIME_RANGE_NINETY].endTimestamp
    if (timestamp >= timeframeStart and timestamp < timeframeEnd) then validRange = true end
  end
  if MasterMerchant.systemSavedVariables.windowTimeRange == LGS_WINDOW_TIME_RANGE_CUSTOM then
    timeframeStart = LGSTime.filterDateRanges[LGS_WINDOW_TIME_RANGE_CUSTOM].startTimestamp
    timeframeEnd = LGSTime.filterDateRanges[LGS_WINDOW_TIME_RANGE_CUSTOM].endTimestamp
    if (timestamp >= timeframeStart and timestamp < timeframeEnd) then validRange = true end
  end
  return validRange
end
...
function MMScrollList:FilterScrollList()
  ...
  local rankIndex = MasterMerchant.systemSavedVariables.rankIndex or LGS_DATERANGE_TODAY
  ...
  elseif MasterMerchant.systemSavedVariables.viewSize == GUILDS then
    ...
    if MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'buyer' then
      if MasterMerchant.salesViewMode == MasterMerchant.personalSalesViewMode then
        dataSet = MMBuyer.playerBuyers
      else
        dataSet = MMBuyer.buyers
      end
    elseif MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'seller' then
      dataSet = MMSeller.sellers
    else
      if MasterMerchant.salesViewMode == MasterMerchant.personalSalesViewMode then
        dataSet = MMPlayerItems.items
      else
        dataSet = MMItems.items
      end
    end

    if searchText == nil or searchText == '' then
      if MasterMerchant.salesViewMode == MasterMerchant.personalSalesViewMode and MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'seller' then
        -- my guild sales
        for gn, g in pairs(dataSet) do
          local sellerData = g[GetDisplayName()] or nil
          local guildData = MMGuild.guilds[gn]
          if (sellerData and sellerData.sales[rankIndex]) then
            if (sellerData.sales[rankIndex] > 0) or (zo_plainstrfind(MMGuild.guildList, gn)) then
              table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, sellerData.sellerName, sellerData.sales[rankIndex], sellerData.rank[rankIndex], sellerData.count[rankIndex], sellerData.stack[rankIndex], guildData and guildData.sales[rankIndex] or 0, sellerData.tax[rankIndex], false }))
            end
          else
            if zo_plainstrfind(MMGuild.guildList, gn) then
              table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, GetDisplayName(), 0, 9999, 0, 0, guildData and guildData.sales[rankIndex] or 0, 0, false }))
            end
          end
        end
      else
        ...
        for gn, g in pairs(dataSet) do
          local guildData = MMGuild.guilds[gn]
          if (MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'seller') then
            if (((guildData and guildData.sales[rankIndex]) or 0) > 0) or (zo_plainstrfind(MMGuild.guildList, gn)) then
              table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, GetString(MM_ENTIRE_GUILD), guildData and guildData.sales[rankIndex] or 0, 0, guildData and guildData.salesCount[rankIndex] or 0, guildData and guildData.salesStack[rankIndex] or 0, guildData and guildData.sales[rankIndex] or 0, guildData and guildData.salesTax[rankIndex] or 0 }))
            end
          elseif (MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'buyer') then
            if MasterMerchant.salesViewMode ~= MasterMerchant.personalSalesViewMode then
              if (((guildData and guildData.purchases[rankIndex]) or 0) > 0) or (zo_plainstrfind(MMGuild.guildList, gn)) then
                table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, GetString(MM_ENTIRE_GUILD), guildData and guildData.purchases[rankIndex] or 0, 0, guildData and guildData.purchaseCount[rankIndex] or 0, guildData and guildData.purchaseStack[rankIndex] or 0, guildData and guildData.purchases[rankIndex] or 0, 0 }))
              end
            end
          end
          for sn, sellerData in pairs(g) do
            if MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'buyer' then
              if (sellerData and sellerData.purchases[rankIndex]) then
                if (sellerData.purchases[rankIndex] > 0) or (zo_plainstrfind(MMGuild.guildList, gn)) then
                  table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, sellerData.buyerName, sellerData.purchases[rankIndex], sellerData.rank[rankIndex], sellerData.count[rankIndex], sellerData.stack[rankIndex], guildData and guildData.purchases[rankIndex] or 0, 0, sellerData.outsideBuyer }))
                end
              else
                --table.insert(listData, ZO_ScrollList_CreateDataEntry(1, {gn, sellerData.buyerName, 0, 9999, 0, 0, guildData and guildData.purchases[rankIndex] or 0, 0, false}))
              end
            elseif MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'seller' then
              if (sellerData and sellerData.sales[rankIndex]) then
                if (sellerData.sales[rankIndex] > 0) or (zo_plainstrfind(MMGuild.guildList, gn)) then
                ...
                --table.insert(listData, ZO_ScrollList_CreateDataEntry(1, {gn, sellerData.sellerName, 0, 9999, 0, 0, guildData and guildData.sales[rankIndex] or 0, 0, false}))
              end
            else
              if (sellerData and sellerData.sales[rankIndex]) then
                if (sellerData.sales[rankIndex] > 0) or (zo_plainstrfind(MMGuild.guildList, gn)) then
                  table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, sellerData.itemLink, sellerData.sales[rankIndex], sellerData.rank[rankIndex], sellerData.count[rankIndex], sellerData.stack[rankIndex], guildData and guildData.sales[rankIndex] or 0, 0, false }))
                end
              else
                --table.insert(listData, ZO_ScrollList_CreateDataEntry(1, {gn, sellerData.itemLink, 0, 9999, 0, 0, guildData and guildData.sales[rankIndex] or 0, 0, false}))
              end
            end
          end
        end
      ...
      if MasterMerchant.salesViewMode == MasterMerchant.personalSalesViewMode and MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'seller' then
        ...
        for gn, g in pairs(dataSet) do
          ...
          if matchesAll and MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'seller' then
            if (((guildData and guildData.sales[rankIndex]) or 0) > 0) or (zo_plainstrfind(MMGuild.guildList, gn)) then
              table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, GetString(MM_ENTIRE_GUILD), guildData and guildData.sales[rankIndex] or 0, 0, guildData and guildData.salesCount[rankIndex] or 0, guildData and guildData.salesStack[rankIndex] or 0, guildData and guildData.sales[rankIndex] or 0, guildData and guildData.salesTax[rankIndex] or 0, false }))
            end
          elseif matchesAll and MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'buyer' then
            if (((guildData and guildData.purchases[rankIndex]) or 0) > 0) or (zo_plainstrfind(MMGuild.guildList, gn)) then
              table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, GetString(MM_ENTIRE_GUILD), guildData and guildData.purchases[rankIndex] or 0, 0, guildData and guildData.purchaseCount[rankIndex] or 0, guildData and guildData.purchaseStack[rankIndex] or 0, guildData and guildData.purchases[rankIndex] or 0, 0, false }))
            end
          end
          for sn, sellerData in pairs(g) do
            ...
            for searchWord in searchByWords do
              ...
              elseif MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'buyer' then
                txt = zo_strlower(MasterMerchant.concat(gn, sellerData.buyerName))
              else
            ...
            if matchesAll then
              if MasterMerchant.systemSavedVariables.viewGuildBuyerSeller == 'buyer' then
                if (sellerData.purchases[rankIndex] and (sellerData.purchases[rankIndex] > 0)) then
                  table.insert(listData, ZO_ScrollList_CreateDataEntry(1, { gn, sellerData.buyerName, sellerData.purchases[rankIndex], sellerData.rank[rankIndex], sellerData.count[rankIndex], sellerData.stack[rankIndex], guildData and guildData.purchases[rankIndex] or 0, 0, sellerData.outsideBuyer }))
                else
                  --table.insert(listData, ZO_ScrollList_CreateDataEntry(1, {gn, sellerData.buyerName, 0, 9999, 0, 0, guildData and guildData.purchases[rankIndex] or 0, 0, false}))
                end
...
function MasterMerchant:UpdateGuildWindowTimerange(timeRange)
  if not timeRange or timeRange == 0 then timeRange = LGS_WINDOW_TIME_RANGE_DEFAULT end
...
function MasterMerchant:UpdateGuildWindow(rankIndex)
  ...
  if not rankIndex or rankIndex == 0 then rankIndex = LGS_DATERANGE_TODAY end
...
function MasterMerchant:BuildFilterTimerangeDropdown()
  ...
  local timeEntries = {
    { range = LGS_WINDOW_TIME_RANGE_DEFAULT, label = MM_WINDOW_TIME_RANGE_LABEL_DEFAULT },
    { range = LGS_WINDOW_TIME_RANGE_THIRTY, label = MM_WINDOW_TIME_RANGE_LABEL_THIRTY },
    { range = LGS_WINDOW_TIME_RANGE_SIXTY, label = MM_WINDOW_TIME_RANGE_LABEL_SIXTY },
    { range = LGS_WINDOW_TIME_RANGE_NINETY, label = MM_WINDOW_TIME_RANGE_LABEL_NINETY },
    { range = LGS_WINDOW_TIME_RANGE_CUSTOM, label = MM_WINDOW_TIME_RANGE_LABEL_CUSTOM }
  }
...
function MasterMerchant:BuildGuidTimeDropdown()
  ...
  local timeEntries = {
    { range = LGS_DATERANGE_TODAY, label = MM_INDEX_TODAY },
    { range = LGS_DATERANGE_YESTERDAY, label = MM_INDEX_YESTERDAY },
    { range = LGS_DATERANGE_THISWEEK, label = MM_INDEX_THISWEEK },
    { range = LGS_DATERANGE_LASTWEEK, label = MM_INDEX_LASTWEEK },
    { range = LGS_DATERANGE_PRIORWEEK, label = MM_INDEX_PRIORWEEK },
    { range = LGS_DATERANGE_7DAY, label = MM_INDEX_7DAY },
    { range = LGS_DATERANGE_10DAY, label = MM_INDEX_10DAY },
    { range = LGS_DATERANGE_30DAY, label = MM_INDEX_30DAY },
    { range = LGS_DATERANGE_CUSTOM, label = LGSTime.customTimeframeText }
  }

  for _, entry in ipairs(timeEntries) do
    local label = GetString(entry.label)
    if entry.range == LGS_DATERANGE_CUSTOM then label = entry.label end
...
function MasterMerchant:SetupMasterMerchantWindow()
  ...
  -- TODO: Should this be MasterMerchantWindowMenuFooterPriceSwitchButton
  ...
  MasterMerchant.systemSavedVariables.rankIndex = MasterMerchant.systemSavedVariables.rankIndex or LGS_DATERANGE_TODAY
