...
function MasterMerchant:ItemCodeText(itemLink)
  local linkStr = tostring(itemLink)
  
  if not zo_strfind(linkStr, ITEM_LINK_TYPE) then 
    MasterMerchant:dm("Debug", "Not a link: " .. linkStr)
    return linkStr
  end
  
  local core, display = linkStr:match("^|H([^|]+)|h(.-)|h$")
  if not core then return linkStr end
  
  display = (display and display ~= "") and display or ""
  return string.format("--%s--%s--", core, display)
end
