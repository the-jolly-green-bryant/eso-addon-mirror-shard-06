    ...
    if currentGuild then
      ...
      if (extradata.doPostedItems) then
        if not internal.postedItems[currentGuild] then
          internal.postedItems[currentGuild] = MMPosted:New(currentGuild)
        end
        ...
        guild:addPostedByDate(firstsaledataItemLink, saledata.timestamp, saledata.price, saledata.quant, false, nil, searchData)
      end

      if (extradata.doPostedSellers) then
        if not internal.postedSellers[currentGuild] then
          internal.postedSellers[currentGuild] = MMPosted:New(currentGuild)
        end
        local guild = internal.postedSellers[currentGuild]
        guild:addPostedByDate(currentSeller, saledata.timestamp, saledata.price, saledata.quant, false, nil)
      end
