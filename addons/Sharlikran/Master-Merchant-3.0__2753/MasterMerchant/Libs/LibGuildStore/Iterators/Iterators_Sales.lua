...
function internal:addSalesData(theEvent)
  ...
  local guild = MMGuild:GetGuild(eventGuild)
  if guild then
    guild:addSaleByDate(timestamp, newEvent.price, newEvent.quant, false)

    if MMGuild:IsGuildMemberByName(eventGuild, eventBuyer) then
      guild:addPurchaseByDate(timestamp, newEvent.price, newEvent.quant, false)
    end
  end

  local item = MMItems:GetItem(eventGuild, eventItemLink, adderDescConcat)
  if item then item:addItemSaleByDate(timestamp, newEvent.price, newEvent.quant, false) end

  local seller = MMSeller:GetSeller(eventGuild, eventSeller)
  if seller then seller:addSaleByDate(timestamp, newEvent.price, newEvent.quant, false) end

  local buyer
  if isSelfSale then
    buyer = MMBuyer:GetPlayerBuyer(eventGuild, eventBuyer, newEvent.wasKiosk, eventSeller)
  else
    buyer = MMBuyer:GetBuyer(eventGuild, eventBuyer, newEvent.wasKiosk)
  end
  if buyer then buyer:addPurchaseByDate(timestamp, newEvent.price, newEvent.quant, false) end

  if isSelfSale then
    local playerItem = MMPlayerItems:GetItem(eventGuild, eventItemLink, adderDescConcat)
    if playerItem then playerItem:addItemSaleByDate(timestamp, newEvent.price, newEvent.quant, false) end
  end
...
function internal:iterateOverSalesData(itemid, versionid, saleid, prefunc, loopfunc, postfunc, extraData)
  ...
  extraData.waitMilliseconds = (extraData.waitMilliseconds or LEQ_WAIT_TIME_IN_MILLISECONDS_DEFAULT)
  ...
  while (itemid ~= nil) do
    ...
    while (versionid ~= nil) do
      if versiondata and versiondata['sales'] then
        local saledata
        if saleid == nil then
        ...
        while (saleid ~= nil) do
          ...
          if saleid and (GetGameTimeMilliseconds() - checkTime) > extraData.checkMilliseconds then
            local LEQ = LibExecutionQueue:new(extraData.waitMilliseconds)
      ...
      if versiondata and versionlist[versionid] ~= nil and (versiondata['sales'] == nil or versiondata["totalCount"] < 1 or not zo_strmatch(tostring(versionid), "^%d+:%d+:%d+:%d+:%d+")) then
      ...
      if LibGuildStore_SavedVariables["updateAdditionalText"] and not extraData.saleRemoved then
        if itemLink and versiondata then
      ...
      -- Sharlikran
      if extraData.wasAltered and not extraData.saleRemoved and versiondata then
...
function internal:TruncateSalesHistory()
  ...
  local prefunc = function(extraData)
    ...
    extraData.checkMilliseconds = MM_WAIT_TIME_IN_MILLISECONDS_STEADY
    extraData.waitMilliseconds = LEQ_WAIT_TIME_IN_MILLISECONDS_DEFAULT
  ...
  local loopfunc = function(itemid, versionid, versiondata, saleid, saledata, extraData)
    ...
    --[[TODO Does the count get managed correctly
...
function internal:IndexSalesData()
  ...
  local loopfunc = function(itemid, versionid, versiondata, saleid, saledata, extraData)
    ...
    -- TODO: test GenerateSearchText and IndexSearchText to see if they return the same data as original
    -- local testSearchText = internal:GenerateSearchText(currentBuyer, currentSeller, currentGuild, versiondata.itemDesc, versiondata.itemAdderText)
    -- local testReturnedWordCount = internal:IndexSearchText(testSearchText, wordData)
    -- no return
  end
...
function internal:InitSalesHistory()
  ...
  local prefunc = function(extraData)
    extraData.start = GetTimeStamp()
    extraData.checkMilliseconds = MM_WAIT_TIME_IN_MILLISECONDS_BALANCED
    extraData.waitMilliseconds = LEQ_WAIT_TIME_IN_MILLISECONDS_DEFAULT

    extraData.playerName = zo_strlower(GetDisplayName())
    extraData.totalRecords = 0
    extraData.emptySales = 0
    extraData.corruptLookupSales = 0
    extraData.wasAltered = false
  ...
  local loopfunc = function(itemid, versionid, versiondata, saleid, saledata, extraData)
    ...
    if type(saledata) ~= "table"
      ...
      or saledata.quant == nil then

      extraData.emptySales = extraData.emptySales + 1
      return false
    end
    ...
    local currentSeller = internal:GetAccountNameByIndex(saledata.seller)
    local currentBuyer = internal:GetAccountNameByIndex(saledata.buyer)
    local saledataItemLink = versiondata and versiondata.sales and versiondata.sales[saleid] and versiondata.sales[saleid].itemLink
    local currentItemLink = internal:GetItemLinkByIndex(saledataItemLink)

    if currentGuild == nil
      or currentSeller == nil
      or currentBuyer == nil
      or currentItemLink == nil then

      extraData.corruptLookupSales = extraData.corruptLookupSales + 1
      return false
    end

    if not MMGuild:IsCurrentGuild(currentGuild) then
      return false
    end

    local isPlayerSale = zo_strlower(currentSeller) == extraData.playerName
    local searchDataDesc = versiondata.itemDesc or zo_strformat(SI_TOOLTIP_ITEM_NAME, GetItemLinkName(currentItemLink))
    local searchDataAdder = versiondata.itemAdderText or internal:AddSearchToItem(currentItemLink)
    local searchData = searchDataDesc .. ' ' .. searchDataAdder

    local guild = MMGuild:GetGuild(currentGuild)
    if guild then
      guild:addSaleByDate(saledata.timestamp, saledata.price, saledata.quant, false)

      if MMGuild:IsGuildMemberByName(currentGuild, currentBuyer) then
        guild:addPurchaseByDate(saledata.timestamp, saledata.price, saledata.quant, false)
      end
    end
    if saledata.timestamp >= LGSTime.oldestTimestamp then
      local item = MMItems:GetItem(currentGuild, currentItemLink, searchData)
      item:addItemSaleByDate(saledata.timestamp, saledata.price, saledata.quant, false)

      if isPlayerSale then
        local playerItem = MMPlayerItems:GetItem(currentGuild, currentItemLink, searchData)
        if playerItem then
          playerItem:addItemSaleByDate(saledata.timestamp, saledata.price, saledata.quant, false)
        end
      end

      local seller = MMSeller:GetSeller(currentGuild, currentSeller)
      if seller then seller:addSaleByDate(saledata.timestamp, saledata.price, saledata.quant, false) end

      local buyer
      if isPlayerSale then
        buyer = MMBuyer:GetPlayerBuyer(currentGuild, currentBuyer, saledata.wasKiosk, currentSeller)
      else
        buyer = MMBuyer:GetBuyer(currentGuild, currentBuyer, saledata.wasKiosk)
      end
      if buyer then buyer:addPurchaseByDate(saledata.timestamp, saledata.price, saledata.quant, false) end
    end
  ...
  local postfunc = function(extraData)
    ...
    MMItems:SortAllRanks()
    MMSeller:SortAllRanks()
    MMBuyer:SortAllRanks()
    MMGuild:SortAllRanks()
    MMPlayerItems:SortAllRanks()
    ...
    if extraData.emptySales > 0 then
      internal:dm("Info", string.format("InitSalesHistory skipped %d incomplete sales records.", extraData.emptySales))
    end
    if extraData.corruptLookupSales > 0 then
      internal:dm("Info", string.format("InitSalesHistory found %d sales records with missing lookup data. SavedVariables has been corrupted.", extraData.corruptLookupSales))
    end
  end
...
function internal:CleanOutBad()
  ...
  local prefunc = function(extraData)
    extraData.start = GetTimeStamp()
    extraData.checkMilliseconds = MM_WAIT_TIME_IN_MILLISECONDS_BULK
    extraData.waitMilliseconds = LEQ_WAIT_TIME_IN_MILLISECONDS_DEFAULT
