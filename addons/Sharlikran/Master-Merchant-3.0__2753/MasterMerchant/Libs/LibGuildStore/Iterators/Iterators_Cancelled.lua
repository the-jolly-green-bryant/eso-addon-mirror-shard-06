    ...
    if currentGuild then
      ...
      if (extradata.doCancelledItems) then
        if not internal.cancelledItems[currentGuild] then
          internal.cancelledItems[currentGuild] = MMCanceled:New(currentGuild)
        end
        ...
        guild:addCanceledByDate(firstsaledataItemLink, saledata.timestamp, saledata.price, saledata.quant, false, nil, searchData)
      end

      if (extradata.doCancelledSellers) then
        if not internal.cancelledSellers[currentGuild] then
          internal.cancelledSellers[currentGuild] = MMCanceled:New(currentGuild)
        end
        local guild = internal.cancelledSellers[currentGuild]
        guild:addCanceledByDate(currentSeller, saledata.timestamp, saledata.price, saledata.quant, false, nil)
      end
