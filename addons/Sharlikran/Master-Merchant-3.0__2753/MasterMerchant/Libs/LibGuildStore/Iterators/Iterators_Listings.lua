    ...
    if currentGuild then
      ...
      if (extradata.doListedItems) then
        if not internal.listedItems[currentGuild] then
          internal.listedItems[currentGuild] = MMListings:New(currentGuild)
        end
        ...
        guild:addListingByDate(firstsaledataItemLink, saledata.timestamp, saledata.price, saledata.quant, false, nil, searchData)
      end

      if (extradata.doListedSellers) then
        if not internal.listedSellers[currentGuild] then
          internal.listedSellers[currentGuild] = MMListings:New(currentGuild)
        end
        local guild = internal.listedSellers[currentGuild]
        guild:addListingByDate(currentSeller, saledata.timestamp, saledata.price, saledata.quant, false, nil)
      end
