...
function internal:IndexPurchaseData()
  ...
  local prefunc = function(extraData)
    extraData.start = GetTimeStamp()
    extraData.checkMilliseconds = MM_WAIT_TIME_IN_MILLISECONDS_FAST
    ...
    if currentGuild then
      ...
      if (extradata.doPurchasedItems) then
        if not internal.purchasedItems[currentGuild] then
          internal.purchasedItems[currentGuild] = MMPurchases:New(currentGuild)
        end
        ...
        guild:addPurchaseByDate(firstsaledataItemLink, saledata.timestamp, saledata.price, saledata.quant, false, false, searchData, nil)
      end

      if (extradata.doPurchasedBuyer) then
        if not internal.purchasedBuyer[currentGuild] then
          internal.purchasedBuyer[currentGuild] = MMPurchases:New(currentGuild)
        end
        local guild = internal.purchasedBuyer[currentGuild]
        guild:addPurchaseByDate(currentBuyer, saledata.timestamp, saledata.price, saledata.quant, false, false, nil, nil)
      end
