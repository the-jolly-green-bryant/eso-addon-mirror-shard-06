...
local sr_index_test = _G["LibGuildStore_SalesIndex_Test"]
function internal:IndexSearchText(searchText, wordData)
  local function IsBadIndexKey(token)
    if token == "" then return true end
    if token == "&" or token == "," then return true end
    if zo_strmatch(token, "^%d+:?$") then return true end
    if zo_strfind(token, "|", 1, true) then return true end  -- |c... / |r ...
    if zo_strfind(token, "\"", 1, true) then return true end -- literal quote
    if zo_strfind(token, "\\", 1, true) then return true end -- backslash

    return false
  end

  local function IsEnglishStopWord(token)
    return token == "a"
      or token == "of"
      or token == "the"
      or token == "and"
  ...
  local function normalizeToken(token, isSoulGem)
    local tokenIn = token
    local tokenOut = token
    local hadAt = (zo_strsub(tokenOut, 1, 1) == "@")
    if isSoulGem and tokenOut == "(empty)" then tokenOut = "empty" end
    if zo_strfind(tokenOut, "\"", 1, true) then
      tokenOut = zo_strgsub(tokenOut, "\"", "")
    end
    tokenOut = zo_strgsub(tokenOut, "^[%p]+", "")
    tokenOut = zo_strgsub(tokenOut, "[%p]+$", "")
    if hadAt and zo_strsub(tokenOut, 1, 1) ~= "@" then
      tokenOut = "@" .. tokenOut
    end
    if tokenOut ~= tokenIn then
      MasterMerchant:dm("Warn", "normalizeToken changed token: '<<1>>' -> '<<2>>'", tokenIn, tokenOut)
    end
    return tokenOut
  end

  local wordsIndexCount = 0
  local searchByWords = zo_strgmatch(searchText, "%S+")

  local isSoulGem = (zo_strfind(searchText, "soul", 1, true) ~= nil) and (zo_strfind(searchText, "gem", 1, true) ~= nil)

  for token in searchByWords do
    token = normalizeToken(token, isSoulGem)

    if not IsBadIndexKey(token) and not IsEnglishStopWord(token) then
      sr_index_test[token] = sr_index_test[token] or {}
      table.insert(sr_index_test[token], wordData)
      wordsIndexCount = wordsIndexCount + 1
    else
      -- MasterMerchant:dm("Warn", "Invalid token: '<<1>>' for searchText: '<<2>>', wordData: '<<3>>, <<4>>, <<5>>'", token, searchText, wordData[1], wordData[2], wordData[3])
    end
  end

  return wordsIndexCount
end
