...
classes.FmSettings = ZO_Object:Subclass()

local LAM2 = LibStub("LibAddonMenu-2.0")

function classes.FmSettings:New(...)
    local controller = ZO_Object.New(self)
    controller:Init(...)
    return controller
end

function classes.FmSettings:ShouldInclude(itemLink)
	local itemType = GetItemLinkItemType(itemLink)
    local localItemType = self.db.itemFilter[itemType]
    return localItemType == nil or localItemType.include
end

function classes.FmSettings:GetGuildName()
	return self.guildNameToNumberList[self.db.guild]
end

function classes.FmSettings:Init()
    self.name = fm.name.."FmSettings"
    self.defaults = {
        useMM = true,
		running = false,
		useCraftBag = true,
