...
function SV:init()
	...
	self.savedVarsPerAccount = ZO_SavedVars:NewAccountWide("PolloxsDailyQuestTracker_SavedVarsAccount", 1, GetWorldName(), self:getAccountDefaults())
	...
	for _, character in ipairs(DQT.Utils:getCharacters()) do
		...
		self.savedVarsPerChar[character.id] = ZO_SavedVars:New("PolloxsDailyQuestTracker_SavedVarsChar",
			1, GetWorldName(), DQT.SV:getCharDefaults(), nil, nil, character.name, character.id, ZO_SAVED_VARS_CHARACTER_ID_KEY)
	end
...
function SV:getAccountDefaults()
	...
	local customColors = {
		toggle = false,
		colorChecked = {r = 1, g = 1, b = 1},
		colorUnchecked = {r = 1, g = 1, b = 1},
		colorDisabled = {r = 1, g = 1, b = 1},
	}
